...
public class Singleton {
    ...
    private String BASE_URL(String ip) {
        return "http://" + ip + "/TicketGo/backend/web/api/";
    }
    ...
    public void verBilhetes(Context context, Response.Listener<JSONArray> listener, Response.ErrorListener errorListener) {
        if (!isNetworkAvailable(context)) {
            ...
            JsonArrayRequest req = new JsonArrayRequest(Request.Method.GET, BASE_URL(ip) + "bilhete", null, listener, errorListener);
            volleyQueue.add(req);
        }
    }
    ...
    public void verZonas(Context context, Response.Listener<JSONArray> listener, Response.ErrorListener errorListener) {
        if (!isNetworkAvailable(context)) {
            ...
            SharedPreferences prefs = context.getSharedPreferences("IP", Context.MODE_PRIVATE);
            String ip = prefs.getString("ip", BaseIp);
            JsonArrayRequest req = new JsonArrayRequest(Request.Method.GET, BASE_URL(ip) + "zona", null, listener, errorListener);
            volleyQueue.add(req);
        }
    }
    ...
    public void verEventos(Context context, Response.Listener<JSONArray> listener, Response.ErrorListener errorListener) {
        if (!isNetworkAvailable(context)) {
            ...
            SharedPreferences prefs = context.getSharedPreferences("IP", Context.MODE_PRIVATE);
            String ip = prefs.getString("ip", BaseIp);
            JsonArrayRequest req = new JsonArrayRequest(Request.Method.GET, BASE_URL(ip) + "evento", null, listener, errorListener);
            volleyQueue.add(req);
        }
    }
    ...
    public void verLocais(Context context, Response.Listener<JSONArray> listener, Response.ErrorListener errorListener) {
        if (!isNetworkAvailable(context)) {
            ...
            SharedPreferences prefs = context.getSharedPreferences("IP", Context.MODE_PRIVATE);
            String ip = prefs.getString("ip", BaseIp);
            Log.d("ip",BASE_URL(ip));
            JsonArrayRequest req = new JsonArrayRequest(Request.Method.GET, BASE_URL(ip) + "local", null, listener, errorListener);
            volleyQueue.add(req);
        }
    }
    ...
    public void verCategorias(Context context, Response.Listener<JSONArray> listener, Response.ErrorListener errorListener) {
        if (!isNetworkAvailable(context)) {
            ...
            SharedPreferences prefs = context.getSharedPreferences("IP", Context.MODE_PRIVATE);
            String ip = prefs.getString("ip", BaseIp);
            JsonArrayRequest req = new JsonArrayRequest(Request.Method.GET, BASE_URL(ip) + "categoria", null, listener, errorListener);
            volleyQueue.add(req);
        }
    }
    ...
    public void verDetalhesEvento(Context context, int id, Response.Listener<JSONObject> listener, Response.ErrorListener errorListener) {
        ...
        SharedPreferences prefs = context.getSharedPreferences("IP", Context.MODE_PRIVATE);
        String ip = prefs.getString("ip", BaseIp);
        String url = BASE_URL(ip) + "evento/" + id;
    ...
    private void obterNomeLocal(String ip, int localId, JSONObject response, Response.Listener<JSONObject> listener, Response.ErrorListener errorListener) {
        String url = BASE_URL(ip) + "local/" + localId;
    ...
    private void obterNomeCategoria(String ip, int categoriaId, JSONObject response, Response.Listener<JSONObject> listener, Response.ErrorListener errorListener) {
        String url = BASE_URL(ip) + "categoria/" + categoriaId;
    ...
    public void pesquisarEventos(Context context, String query, Response.Listener<JSONObject> listener, Response.ErrorListener errorListener) {
        if (!isNetworkAvailable(context)) {
            ...
            JsonObjectRequest req = new JsonObjectRequest(Request.Method.GET, BASE_URL(ip) + "evento/search?query=" + query, null, listener, errorListener);
            Log.d("API", "URL da pesquisa: " + BASE_URL(ip) + "evento/search?query=" + query);
            volleyQueue.add(req);
        }
    }
    ...
    public void adicionarEventoFavoritos(Context context, int eventoId, Response.Listener<JSONObject> listener, Response.ErrorListener errorListener) {
        if (!isNetworkAvailable(context)) {
            ...
            SharedPreferences prefs = context.getSharedPreferences("IP", Context.MODE_PRIVATE);
            String ip = prefs.getString("ip", BaseIp);
            JsonObjectRequest req = new JsonObjectRequest(Request.Method.POST, BASE_URL(ip) + "favoritos/addfavorito/" + eventoId, null, listener, errorListener);
            volleyQueue.add(req);
        }
    }
    ...
    public void removerEventoFavoritos(Context context, int eventoId, Response.Listener<JSONObject> listener, Response.ErrorListener errorListener) {
        if (!isNetworkAvailable(context)) {
            ...
            SharedPreferences prefs = context.getSharedPreferences("IP", Context.MODE_PRIVATE);
            String ip = prefs.getString("ip", BaseIp);
            JsonObjectRequest req = new JsonObjectRequest(Request.Method.DELETE, BASE_URL(ip) + "favoritos/deletefavorito/" + eventoId, null, listener, errorListener);
            volleyQueue.add(req);
        }
    }
    ...
    public void verFavoritosAPI(Context context, SQLiteDatabase db, Response.Listener<JSONArray> listener, Response.ErrorListener errorListener) {
        if (isNetworkAvailable(context)) {
            // Buscar favoritos da API
            SharedPreferences prefs = context.getSharedPreferences("IP", Context.MODE_PRIVATE);
            String ip = prefs.getString("ip", BaseIp);
            JsonArrayRequest req = new JsonArrayRequest(Request.Method.GET, BASE_URL(ip) + "favorito", null, listener, errorListener);
            volleyQueue.add(req);
        } else {
            // Buscar favoritos da BD local
            verFavoritosOffline(context, db, response -> {
                try {
                    JSONArray favoritosArray = response.getJSONArray("favorito");
                    listener.onResponse(favoritosArray);
                } catch (JSONException e) {
                    errorListener.onErrorResponse(new VolleyError("Erro ao processar favoritos locais"));
                }
            });
        }
    }
