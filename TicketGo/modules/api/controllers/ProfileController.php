...
class ProfileController extends ActiveController
{
    ...
    public function verifyCredentials($token, $profile_id)
    {
        ...
        $user = User::find()
        ->where(['id' => $profile_id, 'auth_key' => $token])
        ->one();
    ...
    // Obtém os dados do corpo da requisição (JSON ou form-data)
    $requestData = Yii::$app->request->getBodyParams();

    // Atualiza os campos do perfil com os dados enviados no corpo da requisição
    $profile->datanascimento = isset($requestData['datanascimento']) ? $requestData['datanascimento'] : $profile->datanascimento;
    $profile->nif = isset($requestData['nif']) ? $requestData['nif'] : $profile->nif;
    $profile->morada = isset($requestData['morada']) ? $requestData['morada'] : $profile->morada;
